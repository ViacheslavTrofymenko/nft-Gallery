...
const Home = () => {
  ...
  if (!collection.length) {
    ...
    const fetchURL = `${baseURL}?owner=${wallet}&contractAddresses%5B%5D=${collection}&pageKey=${"next"}`;
    nfts= await fetch(fetchURL, requestOptions).then(data => data.json())
  } 
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 gap-y-3">
      <div className="flex flex-col w-full justify-center items-center gap-y-2">
        ...
        <label className="text-gray-600 "><input onChange={(e)=>{setFetchForCollection(e.target.checked)}} value={fetchForCollection}type={"checkbox"} className="mr-2"></input>Fetch for collection</label>
